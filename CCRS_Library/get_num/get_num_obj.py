...
# 临时存储PosixPath类，用于后续在Windows系统上的路径处理
# Temporarily store the PosixPath class for path handling on Windows systems later
temp = pathlib.PosixPath
# 在Windows系统上，将PosixPath替换为WindowsPath，以确保路径处理的兼容性
# On Windows systems, replace PosixPath with WindowsPath to ensure path handling compatibility
pathlib.PosixPath = pathlib.WindowsPath
# 初始化模型为None，后续会加载具体的模型
...
def load_model():
    ...
    with importlib.resources.path("CCRS_Library.get_num", "num_obj_v1.pt") as model_path:
        # 加载模型
        # Load the model
...
def compute_skew(image):
    ...
    """
    # 确保图像是灰度的，如果是彩色图像则转换为灰度图像
    # Ensure the image is grayscale. If it is a color image, convert it to grayscale
...
def load_image(path, show_pic=False, show_lowPic=False):
    ...
    # 调整图像大小，放大3倍
    # Resize the image, magnify it by 3 times
...
def main(save_path="./Out_PIC/", save_name='result.jpg', save_file=False, show_result=False, load_imagePath='./cache/crop.jpg'):
    ...
    for r in results:
        # print("Boxes:", r.boxes)  # 打印检测到的边界框
        # print("Names:", r.names)  # 打印类别名称
    ...
    recognized_classes.sort(key=lambda x: x[1], reverse=True)
